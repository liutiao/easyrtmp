...
void CRsCameraDS::CloseCamera()
{
	...
	//jacky, release m_pMediaEvent failed, remark it temporary
//	m_pMediaEvent		= NULL;
	m_pNullFilter		= NULL;
...
BOOL_ CRsCameraDS::OpenCamera(int nCamID, int nWidth, int nHeight, int nFPS, BOOL_ bDisplayProperties)
{
	...
	else 
	{
	   ...
	   {   
			...
			phead->AvgTimePerFrame		= 100000000 / nFPS;
			phead->AvgTimePerFrame		= 1000000 / nFPS;
			//fUseFrameRate
...
BOOL_ CRsCameraDS::BindFilter(int nCamID, IBaseFilter **pFilter)
{
	if (nCamID < 0)
		return FALSE_;
...
*/
#ifdef _JACKY_USING_OPENCV_
IplImage* CRsCameraDS::QueryFrame()
#else
BYTE* CRsCameraDS::QueryFrame()
#endif
{
	long evCode;
	long size = 0;
...
int CRsCameraDS::CameraName(int nCamID, char* sName, int nBufferSize)
{
	int count = 0;
 	CoInitialize(NULL);
